let audioPlayer; // contiene il player 
let currentSong = {"title": "selezionare uan canzone "}; // contiene la song corrent
...
const Track = function (track, i) {
	return `
		  <li class="list-group-item" onclick="loadTrackById(${track.id})" id="track-${track.id}">
  ...
  <main>
	...
	<div class="details">
	  ...
	  <div class="album-meta">
		...
		<img src="${
			currentAlbum.artist.picture_small
...
const trackSectionMobile = function () {
	...
	for (const track of currentAlbum.tracks.data) {
		TracklistMobile += songToHtmlTrack(track);
	}
...
const setCurrentSong = function (track){
	currentSong = track;
}
...
const htmlTrackToSong = function(id){
	const trackHTML = document.getElementById(`track-${id}`)
	return {
		"album_id": +(trackHTML.querySelector('.track-album-id').value),
		"id": +(trackHTML.querySelector('.track-id-hidden').value),
		"title": trackHTML.querySelector('.track-title-hidden').value,
		"preview": trackHTML.querySelector('.track-preview').value,
	}
}

const songToHtmlTrack = function (track) {
	const playing = currentSong.id === track.id ? 'song-playing':'';
	return `
  <div class="track-item ${playing}" id="track-${track.id}" onclick="loadTrackById(${track.id})">
...
const Player = function () {
	/* se il player audio e` messo in pausa mettiamo il pottone pausa 
	 * senno il bottone play */
	const tastoPlay = audioPlayer?.paused ? "bi-play-fill":"bi-pause-fill" 
	return `
   <div class="mini-player-wrapper">
	...
	<div class="mini-actions">
	  ...
	  <i class="bi bi-heart"></i>
	  <button onclick="togglePlay()" class="mini-play-btn" aria-label="Play/Pause">
	    <i class="bi ${tastoPlay}"></i>
	  </button>
	</div>
      <div id="mini-progress" class="mini-progress"></div>
      </div>
    </div> 
     ${FooterNav()}
  `;
...
const FooterNav = function () {
  ...
  <nav class="bottom-nav" role="navigation" aria-label="Bottom navigation">
      <a href="#" class="nav-item" aria-label="Home" onclick=setPage(0)>
	<i class="bi bi-house-door-fill" aria-hidden="true"></i>
	<span class="nav-label" >Home</span>
      </a>
      <a href="#" class="nav-item" aria-label="Cerca" onclick=setPage(3)>
	<i class="bi bi-search" onclick="" aria-hidden="true"></i>
